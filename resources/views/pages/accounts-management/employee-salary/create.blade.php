@extends('layouts.app', ['activePage' => 'employeeSalary', 'titlePage' => __('Employee Salary Entry')])
...
@section('content')
  <div class="content">
    <div class="container-fluid">
      ...
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('employeeSalary.store') }}" autocomplete="off" class="form-horizontal">
            @csrf
            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Employee Salary Entry') }}</h4>
                <p class="card-category">{{ __('Employee Salary Entry information') }}</p>
              </div>
              <div class="card-body ">
                      ...
                      <select class="form-control" name="employee_id" required>
                        ...
                        @if ($employees)
                          ...
                          @foreach ($employees as $employee)
                            ...
                            <option value="{{$employee->id}}">{{$employee->first_name}} {{$employee->last_name}} - {{$employee->id_no}} </option>
